...
| R1-M3 / R2-M5 | Dense conversion and scalability | **REPLY-READY (qualified implementation-level)** | D13 seed-0 five-point series (10k/25k/50k/100k/full), corrected D16 full profile, D5/D11 training batch grid (24 rows) and frozen-checkpoint inference batch/order diagnostic (110 rows) all PASS; D13 full peak CPU RSS ~202.9 GiB and D16 profile ~207.84 GiB, with GPU allocated/reserved reported separately. | We added implementation-scale and batch/order diagnostics. CPU-side preprocessing is the principal memory bottleneck; the released batch-local graph is sensitive to scale, batch size and cell order, so we report feasibility with explicit limitations. | Linear runtime, uniformly stable sizes, low total memory, or batch/order-invariant assignments; do not generalize one-seed D13/D16 to all datasets. | `reports/R1_MAJOR3_R2_MAJOR5_SCALABILITY_REPORT.md`; `08_scalability/{d13_scaling_series,stage_profile,training_batch_size,training_batch_size_assignment_stability}.csv`; D13/D16 sparse-safe/global-graph and unified batch integration remain missing |
...
| R1-M5 | Batch correction workflow | **PARTIAL (D16 profile PASS)** | Corrected D16 full profile and D13 scaling now quantify implementation memory; unified D13–D16 batch-integration/MOFA+ remains unrun. | We report the D16 preprocessing bottleneck and keep the proposed metacell-then-common-MOFA workflow explicit pending the unified benchmark. | Improved batch correction or a completed unified integration claim | `reports/R1_M5_D16_FULL_PROFILE_REPORT.md`; `reports/R1_MAJOR3_R2_MAJOR5_SCALABILITY_REPORT.md`; unified MOFA+ missing |
